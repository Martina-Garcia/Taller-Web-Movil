"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkersController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const create_worker_dto_1 = require("./dto/create-worker.dto");
let WorkersController = class WorkersController {
    create(createWorkerDto) {
    }
    findAll() {
    }
    findOne(id) {
    }
};
exports.WorkersController = WorkersController;
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Registrar un nuevo trabajador (Picker o Supervisor)' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'El trabajador ha sido creado exitosamente.' }),
    (0, swagger_1.ApiResponse)({ status: 400, description: 'Datos inválidos.' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_worker_dto_1.CreateWorkerDto]),
    __metadata("design:returntype", void 0)
], WorkersController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Listar todos los trabajadores activos e inactivos' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Retorna un arreglo de trabajadores.' }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], WorkersController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Obtener el detalle de un trabajador por su ID' }),
    (0, swagger_1.ApiParam)({ name: 'id', description: 'ID numérico del trabajador' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Retorna los datos del trabajador.' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Trabajador no encontrado.' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], WorkersController.prototype, "findOne", null);
exports.WorkersController = WorkersController = __decorate([
    (0, swagger_1.ApiTags)('Trabajadores'),
    (0, common_1.Controller)('workers')
], WorkersController);
//# sourceMappingURL=workers.controller.js.map